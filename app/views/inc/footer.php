</div>
<footer>
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <nav class="nav-footer">
            <ul class="list-inline">
              <li class="list-inline-item">
                <a href="<?php echo URLROOT; ?>">Home</a>
              </li>
              <li class="list-inline-item">
                <a href="<?php echo URLROOT; ?>/pages/about">About</a>
              </li>
              <li class="list-inline-item">
                <a href="<?php echo URLROOT; ?>/lodges/showAll">Lodges</a>
              </li>
              <li class="list-inline-item">
                <a href="<?php echo URLROOT; ?>/pages/contact">Contact</a>
              </li>
              <li class="list-inline-item">
                <a href="<?php echo URLROOT; ?>/reservations/changeReservation">Reservations</a>
              </li>

            </ul>
          </nav>
          <div class="copyright-footer">
            <p class="copyright color-text-a">
              &copy; Copyright
              <span class="color-a"><?php echo SITENAME; ?></span> All Rights Reserved.
            </p>
          </div>
          <div class="credits">
            Designed by Placide with <i class="fa fa-heart"></i>
          </div>
        </div>
      </div>
    </div>
  </footer><!-- End  Footer -->

<!-- <script type="text/javascript" src="<?php echo URLROOT; ?>/js/new.js"></script> -->
<script type="text/javascript" src="<?php echo URLROOT; ?>/js/main.js"></script>
<script type="text/javascript" src="<?php echo URLROOT; ?>/js/jquery-3.5.1.js"></script>
<script type="text/javascript" src="<?php echo URLROOT; ?>/js/bootstrap.js"></script>
<script src="<?php echo URLROOT; ?>/js/jquery/jquery.min.js"></script>
  <script src="<?php echo URLROOT; ?>/js/bootstrap/js/bootstrap.bundle.min.js"></script>
  <script src="<?php echo URLROOT; ?>/js/jquery.easing/jquery.easing.min.js"></script>
  <script src="<?php echo URLROOT; ?>/js/php-email-form/validate.js"></script>
  <script src="<?php echo URLROOT; ?>/js/owl.carousel/owl.carousel.min.js"></script>
  <script src="<?php echo URLROOT; ?>/js/scrollreveal/scrollreveal.min.js"></script>

  <!-- datables -->
<script type="text/javascript" src="<?php echo URLROOT; ?>/js/datatables/jquery.dataTables.js"></script>
<script type="text/javascript" src="<?php echo URLROOT; ?>/js/datatables/dataTables.bootstrap4.js"></script>

  <!-- Template Main JS File -->
  <script src="<?php echo URLROOT; ?>/js/home.js"></script>
</body>
</html>